...
def preprocess(
    ...
    for traj in trajectory_list:
        for t in range(lookback + 1, traj.shape[0] - lookforward):
            ...
            for j in range(len(dims_to_diff)):
                if dims_to_diff[j]:
                    res_X.append(np.diff(traj[t - lookback - 1 : t, j], axis=0).flatten(order='F'))
                    res_y.append(np.diff(traj[t - 1 : t + lookforward, j], axis=0).flatten(order='F'))
                else:
                    res_X.append(traj[t - lookback : t, j].flatten(order='F'))
                    res_y.append(traj[t : t + lookforward, j].flatten(order='F'))
    ...
    for traj in trajectory_list:
        for t in range(lookback + 1, traj.shape[0] - lookforward):
            X.append(np.diff(traj[t - lookback - 1 : t, :], axis=0).flatten(order='F'))
            # import pdb; pdb.set_trace()
            y.append(np.diff(traj[t - 1 : t + lookforward, :], axis=0).flatten(order='F'))
            # y.append(traj[t, :] - traj[t - 1, :])
    """
